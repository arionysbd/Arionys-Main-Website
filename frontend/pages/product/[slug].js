...
export default function BlogPage({ blog }) {
    return (
        <>
            ...
            <Head>
                <title>{blog.title ? `${blog.title} | Arionys` : "Loading..."}</title>
                <meta name="description" content={blog.metadescription || "Discover our latest robotics projects, advancements, and blog posts on Arionys."} />
                <meta name="keywords" content={`${blog.keywords || "Technology, Robotics, AI, Blog, Robotics Projects"}`} />
                <meta name="author" content={blog.author || "Arionys"} />
                <meta name="viewport" content="width=device-width, initial-scale=1.0" />

                {/* Open Graph */}
                <meta property="og:locale" content="en_US" />
                <meta property="og:type" content="article" />
                <meta property="og:title" content={`${blog.title} | Arionys`} />
                <meta property="og:description" content={blog.metadescription || "Explore the latest updates and innovations in robotics on Arionys."} />
                <meta property="og:url" content={`${process.env.SITE_URL}/product/${blog.slug}`} />
                ...
                <meta name="twitter:title" content={blog.title || "Arionys - Blog"} />
                <meta name="twitter:description" content={blog.metadescription || "Stay up to date with the latest news and breakthroughs in robotics at Arionys."} />
                <meta name="twitter:image" content={extractFirstImageUrl(blog.description) || '/img/projectthum.png'} />

                {/* Canonical URL */}
                <link rel="canonical" href={`${process.env.SITE_URL}/product/${blog.slug}`} />
                ...
                <script type="application/ld+json">
                    {`{
                        ...
                        "mainEntityOfPage": {
                            "@type": "WebPage",
                            "@id": "${process.env.SITE_URL}/product/${blog.slug}"
                        },
                        "headline": "${blog.title}",
            ...
            <div className="slugpage">
                <div className="container">
                    <div className="topslug_titles" data-aos="fade-right">
                        ...
                        <h5>
                            By <span>Arionys</span>・ Published in{" "}
...
export async function getServerSideProps(context) {
    ...
    const res = await fetch(
        `${baseUrl}/api/getproduct?slug=${slug}`
    );
